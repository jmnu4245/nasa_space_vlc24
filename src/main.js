...
// Función para cargar los dos archivos JSON
Promise.all([
  fetch('./asteroids2_ordenado.json'),
  fetch('./comets2_ordenado.json')
])
  .then(responses => {
    ...
    return Promise.all(responses.map(response => response.json())); // Convierte ambas respuestas a JSON
  })
  .then(data => {
    // data es un array con los datos de ambos archivos
    main(data[0], data[1]);
  })
  .catch(error => {
    console.error('Hubo un problema con la carga de los archivos JSON:', error);
  });
...
function main(datos_asteroides, datos_cometas){
  //Inicializar escena
  ...
  const planetaso = [
    new CelestialBody('0', 'Sol', 0, 0, 0, 0, 0, 0, 0, 0, 0, 24, 1391000,'../texturas/textura_sol.jpg',0xffe59e),
    new CelestialBody('1', 'Mercurio', 0.2056, 0.387, 0.3075, 7.0, 48.3, 77, 252.25, 0, 4.15, 58.6, 4880, '../texturas/textura_mercurio.png',0xc2c2c2),
    new CelestialBody('2', 'Venus', 0.0068, 0.723, 0.718, 3.4, 76.7, 131.6, 181.98, 0, 1.62, -243, 12104, '../texturas/textura_venus.jpg',0xffd5a0), //rotación retrógrada
    new CelestialBody('3', 'Tierra', 0.0167, 1.000, 0.983, 0.0, 0.0, 102.9, 100.46, 0, 0.99, 1, 12742, '../texturas/textura_tierra.jpg',0xa3c1e0),
    new CelestialBody('4', 'Marte', 0.0934, 1.524, 1.381, 1.9, 49.6, -23.96, -4.55, 0, 0.53, 1.03, 6780,'../texturas/textura_marte.jpg',0xffb3b3 ),
    new CelestialBody('5', 'Júpiter', 0.0484, 5.203, 4.951, 1.3, 100.5, 14.73, 34.40, 0, 0.08, 0.41, 139822,'../texturas/textura_jupiter.jpg',0xffd5a0),
    new CelestialBody('6', 'Saturno', 0.0542, 9.537, 9.020, 2.5, 113.7, 92.6, 49.95, 0, 0.03, 0.45, 116464,'../texturas/textura_saturno.jpg',0xffe59e),
    new CelestialBody('7', 'Urano', 0.0472, 19.191, 18.286, 0.8, 74.0, 170.95, 313.24, 0, 0.01, -0.72, 50724,'../texturas/textura_urano.jpg',0xb3e0e0), //rotación retrógrada
    new CelestialBody('8', 'Neptuno', 0.0086, 30.069, 29.819, 1.8, 131.8, 44.96, -55.12, 0, 0.006, 0.67, 49244,'../texturas/textura_neptuno.jpg',0xa3c1e0)
  ];

  recopilar_anadir_datos(planetaso,datos_asteroides,10);
  recopilar_anadir_datos(planetaso,datos_cometas,0);
  

  let sphere = Array(planetaso.length).fill(0);
  let orbitas = Array(planetaso.length-1).fill(0); // Solo 8 órbitas, una por planeta (excluyendo el Sol)
...
function recopilar_anadir_datos(lista_planetas, datos, n) {
  datos.slice(0, n).forEach(body => {
    const astro = new CelestialBody(
        body.spkid,
        body.full_name,
        body.e,
        body.a,
        ...
        body.rot_per,
        body.diameter,
        '../texturas/textura_asteroide.jpg', 
        0xffffff                            
    );
    lista_planetas.push(astro);
  });
}
